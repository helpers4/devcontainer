...
# ubuntu:latest, even though mistral-dev resolved a non-root "ubuntu" user
...
if [ ! -L "${TARGET}" ] || [ "$(readlink -f "${TARGET}")" != "/mnt/h4vibe" ]; then
    echo "❌ FAIL: ${TARGET} is not a symlink to /mnt/h4vibe"
    exit 1
fi
...
# mistral-dev resolved (true on the mcr.microsoft.com/devcontainers/base:ubuntu
